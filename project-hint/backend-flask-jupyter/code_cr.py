...
def select_same_industry(corp_name):
    ...
    # print(com_df.groupby(by='industry')['nm'].nunique().max())
...
def relate_code_crawl(gcode):
    ...
    # page = pd.read_html(url, encoding='euc-kr')
...
def yh_code_to_bs_gicode(yh_code):
    gi = com_df[com_df['yh_code'] == yh_code]['cd']
    gi = gi.values[0]
    return gi
...
def bs_craw(stock_code, clear_name=False):
    ...
    # ========== 검색과 연동해서 입력되는 변수dp 따라 gicode(네이버에서 분류하는 기업 코드)로 변환
    ...
    # table_list = pd.read_html(url, encoding='cp949')
...
# ======================================================================== 네이버 금융
# ========== kind로 특정 테이블 지정하는 대신 df list 전체 반환
...
def fn_craw(stock_code):
    ...
    table_list = pd.read_html('https://finance.naver.com/item/main.naver?code=' + gcode, encoding='cp949')
...
# ======================================================================== 지표 선정
# =============== 데이터프레임 하나가 아닌 리스트로 받아오기때문에 kind 제거하고 직접 선택해줌
# ========== sli_df_y, sil_df_q 에서 '-' 가공 시 if 조건에 따라 처리하는 대신 lambda와 re.sub 이용
# ===== dict 대신 array로 반환, 기업 이름(nm 값도 반환)
...
def relate_radar_weather_data(stock_code):
    ...
    relate_corp = relate_code_crawl(gcode)
    print(relate_corp)
...
#     return label_list, radar_dict_list, weather_list, foreign_ms, giguan_ms
...
def idv_radar_data(stock_code):
    ...
    if (sil_df.iloc[0:8, 3].isna().sum()) > 0:  # 표 안 가르고 계산하는 건 신규 상장 기업은 정보가 아예 없기 때문
        ...
        # ========================================================================
        # ========== 0) 재무정보는 최신 분기 실공시 기준
        # ===== 0-1) 배당은 1년에 한 번 이루어지기 때문에 최신 년도 공시 기준
        
        sil_df_y = sil_df['최근 연간 실적'].iloc[:, 2]  # .iloc >>> 공시 날짜가 다른 기업이 있기 때문
        sil_df_q = sil_df['최근 분기 실적'].iloc[:, 4]
        ...
        # ========================================================================
        # ========== 4) 수익성 분석 - 매출수익성(당기순이익/매출액) # TODO 매출액 0인 기업 처리 필요
...
# ======================================================================== 시각화
# ========== 매출, 당기순이익 추이
# ===== 데이터는 list로 받아오므로 kind 없애고 직접 인덱스 처리
...
def icon_selection(index_array):
    ...
    if 3 < index_array[0]:
        res.append("RAIN")
    elif (1.2 < index_array[0] and index_array[0] <= 3):
        res.append("CLOUDY")
    elif (0.8 < index_array[0] and index_array[0] <= 1.2):
        res.append("PARTLY_CLOUDY_DAY")
    elif (0 < index_array[0] and index_array[0] <= 0.8):
        res.append("CLEAR_DAY")
    else:
        res.append("SNOW")
    ...
    if 3 < index_array[1]:
        res.append("RAIN")
    elif (1.2 < index_array[1] and index_array[1] <= 3):
        res.append("CLOUDY")
    elif (0.8 < index_array[1] and index_array[1] <= 1.2):
        res.append("PARTLY_CLOUDY_DAY")
    elif (0 < index_array[1] and index_array[1] <= 0.8):
        res.append("CLEAR_DAY")
    else:
        res.append("SNOW")
    ...
    if 3 < index_array[2]:
        res.append("CLEAR_DAY")
    elif (1.2 < index_array[2] and index_array[2] <= 3):
        res.append("CLEAR_DAY")
    elif (0.8 < index_array[2] and index_array[2] <= 1.2):
        res.append("PARTLY_CLOUDY_DAY")
    elif (0 < index_array[2] and index_array[2] <= 0.8):
        res.append("CLOUDY")
    else:
        res.append("RAIN")
    ...
    if 3 < index_array[3]:
        res.append("CLEAR_DAY")
    elif (1.2 < index_array[3] and index_array[3] <= 3):
        res.append("CLEAR_DAY")
    elif (0.8 < index_array[3] and index_array[3] <= 1.2):
        res.append("PARTLY_CLOUDY_DAY")
    elif (0 < index_array[3] and index_array[3] <= 0.8):
        res.append("CLOUDY")
    else:
        res.append("RAIN")
    ...
    if 3 < index_array[0]:
        res.append("CLEAR_DAY")
    elif (1.2 < index_array[4] and index_array[4] <= 3):
        res.append("CLEAR_DAY")
    elif (0.8 < index_array[4] and index_array[4] <= 1.2):
        res.append("PARTLY_CLOUDY_DAY")
    elif (0 < index_array[4] and index_array[4] <= 0.8):
        res.append("CLOUDY")
    else:
        res.append("RAIN")
...
def news_crawl(gi):
    ...
    for p in range(1):
        ...
        # https://m.stock.naver.com/domestic/stock/000020/total
...
#     url = 'https://finance.naver.com/item/coinfo.naver?code=' + gcode
    page = pd.read_html(url, encoding='cp949')
#     page = pd.read_html(url, encoding='euc-kr')
#     page = pd.read_html(url, encoding='UTF-8')
    try:
...
#     ifrs.rename(columns={'IFRS(연결)': ''}, inplace=True)
    ifrs.columns= ['IFRS(연결)', '2019/12', '2020/12', '2021/12', '2022/12(E)']
